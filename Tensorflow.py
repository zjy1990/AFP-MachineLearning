...
def getReturn(action,realize_return):
    if action == "Buy":
        value = realize_return - trans_cost
    elif action == "Hold":
        value = 0
    else:
        value = -realize_return - trans_cost - borrow_rate
    return(value)
...
#run model random time series
    ...
    for step in range(test_data.shape[0]):
        ...
        ptf_value.append((1+getReturn(action,realize_return))*ptf_value[step])
