...
year_of_data = raw_data.Date
year_of_data = pd.DatetimeIndex(year_of_data).year
year_of_data = np.unique(year_of_data)

results_table = pd.DataFrame(data=np.zeros((6,year_of_data[8:-1].shape[0])))
results_table.columns = year_of_data[8:-1]
results_table.index = ['Strategy return','Strategy volatility','Strategy SR','Strategy MDD','Market SR','Prediction accuracy']
#tech firm
...
def main(train_data,test_data,results_table):
    ...
    # plt.legend()
    # plt.show()
    results_table[0,year] = ptf_value[-1]/initial_capital - 1
    results_table[1,year] = ptf_volatility*np.sqrt(252)
    results_table[2,year] = SR_ptf
    results_table[3, year] = MDD
    results_table[4,year] = SR_mkt
    results_table[5, year] = overall_accuracy

for year in year_of_data[8:-1]:
    train_date_start = str(year - 7) + str('-01-01')
    train_date_end = str(year) + str('-12-31')
    test_data_start = str(year) + str('-02-01')  # need to be date whereby the trading start date minus batch_size
    test_data_end = str(year + 1) + str('-12-31')
    train_data = raw_data[(raw_data.Date >= train_date_start) & (raw_data.Date <= train_date_end)]
    test_data = raw_data[(raw_data.Date >= test_data_start) & (raw_data.Date <= test_data_end)]
    print("Year :"+str(year) +"!!!")
    main(train_data=train_data,test_data=test_data, results_table = results_table)